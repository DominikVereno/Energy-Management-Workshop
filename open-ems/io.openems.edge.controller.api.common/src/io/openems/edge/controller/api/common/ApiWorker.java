...
public class ApiWorker {
	...
	public CompletableFuture<JsonrpcResponseSuccess> handleSetChannelValueRequest(ComponentManager componentManager,
		...
		// set value
		var writeChannel = (WriteChannel<?>) channel;
		var writeObject = new WritePojo(value);

		// Apply once right here, so a Channel that refuses the value reports back to
		// the caller of this request instead of failing silently once per cycle for as
		// long as the value stays queued below.
		if (!writeObject.isNull()) {
			writeObject.setNextWriteValue(writeChannel);
		}

		this.addValue(writeChannel, writeObject);
